...
$hojas = "";
$query = $conexion->query("SELECT idioma.nombre AS nombre, nivel.nivel AS nivel, hoja_trabajo.tema AS titulo, hoja_trabajo.area AS tipo  FROM hoja_trabajo JOIN nivel JOIN idioma WHERE hoja_trabajo.id_nivel = nivel.id_nivel AND nivel.id_idioma = idioma.id_idioma ");
...
if(isset($_POST['titulo']) || isset($_POST['tipo']) || isset($_POST['idioma'])){
    ...
    if($titulo || $tipo ||$idioma != ""){
        ...
        if($titulo != "" && $tipo != "" && $idioma != ""){ //todos rellenados
            $filtro = $conexion->query("SELECT idioma.nombre AS nombre, nivel.nivel AS nivel, hoja_trabajo.tema AS titulo, hoja_trabajo.area AS tipo FROM hoja_trabajo JOIN nivel JOIN idioma
